...
import java.awt.event.KeyEvent; // Read data from the serial port
...
void drawObject() {
    ...
    if(iDistance < 40) line(pixsDistance * cos(radians(iAngle)), -pixsDistance * sin(radians(iAngle)), (width - width * 0.505) * cos(radians(iAngle)), -(width - width * 0.505) * sin(radians(iAngle))); // draws the object according to the angle and the distance
    popMatrix();
}
